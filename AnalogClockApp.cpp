...
void AnalogClockApp::redraw(void) {
    // erase the last dots;
    coord_t x, y;
    uint16_t theta;

    theta = clock.hourFormat12();
    if (theta==12) { theta=0; }
    theta = ((long)theta * 65536L) / 12L;
    dotpos(theta, hr_length, x, y);
    ...
    theta = ((long)clock.minute() * 65536L)/60L;
    dotpos(theta, min_length, x, y);
    ...
    theta = ((long)clock.second() * 65536L)/60L;
    dotpos(theta, sec_length, x, y);
