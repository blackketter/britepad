...
void drawLogo(void) {
  ...
  screen.fillCircle(midwidth-radius, midheight, radius, currentColor);
  screen.fillCircle(midwidth-radius, midheight, radius-weight, backgroundColor);
  screen.fillCircle(midwidth+radius, midheight, radius, currentColor);
  screen.fillCircle(midwidth+radius, midheight, radius-weight, backgroundColor);
  screen.fillRect(midwidth-2*radius, midheight-2*radius, weight, ascender, currentColor);
  screen.fillRect(midwidth, midheight, weight, ascender, currentColor);
}
...
bool trackPadApp(void) {
  ...
  return false;  // never exit trackpad app
}
...
void loop() {
  ...
  static BPApp currApp = BubblesApp();
  pad.update();
  ...
  if (screensaver_started) {
    currApp.run();
  } else if (pad.changed(TOP_PAD)) {
    screen.fillRect(0,0,screen.width()-1,screen.height()-1, pad.touched(TOP_PAD) ? screen.green : backgroundColor);
  } else {
    trackPadApp();
  }
