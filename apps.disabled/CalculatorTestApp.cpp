#include "CalculatorTestApp.h"

CalculatorTestApp theCalculatorTestApp;
...
void CalculatorTestApp::begin(AppMode asMode) {
...
void CalculatorTestApp::end() {
//  delete buttons;
//  buttons = nullptr;
  BritepadApp::end();
}

void CalculatorTestApp::run() {
  Button* hit = nullptr; // = buttons->down();
  if (hit) {
    handleKey(hit->getID());
  }
...
void CalculatorTestApp::acceptText() {
  if (strlen(topText)) {
    pop();
    if (getBase() == 10) {
...
void CalculatorTestApp::keySwap() {
  double a = pop();
  double b = pop();
  push(a);
  push(b);
...
void CalculatorTestApp::keyInverse() {
  if (top() == 0) {
    sound.beep();
  } else {
    push(1/pop());
...
void CalculatorTestApp::keyDivide() {
  if (top() == 0) {
    sound.beep();
  } else {
    double divisor = pop();
...
void CalculatorTestApp::keyAdd() {
  push(pop()+pop());
}

void CalculatorTestApp::keyMultiply() {
  push(pop()*pop());
}

void CalculatorTestApp::keySubtract() {
  double value = pop();
  push(pop()-value);
}

void CalculatorTestApp::keyShift() {
  keyMap curMap = (keyMap)0;//buttons->getMap();
...
void CalculatorTestApp::keyBase() {
  if (getBase() == 10) {
...
void CalculatorTestApp::keyPercent() {
  double fractional = pop();
  double whole = top();
  push(fractional*whole/100);
}

void CalculatorTestApp::keyBackspace() {
...
void CalculatorTestApp::keySend() {
  char textNum[100];
...
void CalculatorTestApp::keyEnter() {
  if (strlen(topText) == 0) {
    push(top());
  } else {
    acceptText();
...
void CalculatorTestApp::keyDigit(uint8_t digit) {
  int len = strlen(topText);
  if (len < maxTopText()) {
    if (len == 0) {
      push(0);
...
void CalculatorTestApp::keyDecimal() {
  int len = strlen(topText);
...
void CalculatorTestApp::drawDisplay() {
...
char* CalculatorTestApp::formatText(char* fstring, double value) {
